...
def nominals(trees, lang, stop_dict, deverbs):
    res = 0
    # Check if lang exists in stop_dict and deverbs
    if lang not in stop_dict or lang not in deverbs:
        print(f"Warning: Missing language data for {lang}")
        return res
    
    stoplst = stop_dict[lang]
    converted = deverbs[lang]

    # Define a function to handle English-specific logic
    def process_english(w, tree):
        nonlocal res
        ...
        if 'NOUN' in w[3] and w[2] in converted:
            if w[1].endswith('ing'):
                return
            kids_pos = get_kids_pos(w, tree)
            if 'DET' not in kids_pos and 'ADJ' not in kids_pos:
                if 'Number=Sing' in w[5]:
                    return
            res += 1

    # Define a function to handle German-specific logic
    def process_german(w):
        nonlocal res
        if w[2] not in stoplst:
    ...
    # Define a function to handle Russian-specific logic
    def process_russian(w):
        nonlocal res
    ...
    # Process trees based on the language
    for tree in trees:
        for w in tree:
            if lang == 'en':
                process_english(w, tree)
            elif lang == 'de':
                process_german(w)
            elif lang == 'ru':
                process_russian(w)
